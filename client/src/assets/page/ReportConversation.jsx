...
const ReportPost = () => {
  ...
  const missingState = !state?.conversationId;
  ...
  const alreadyReportedKey = state?.conversationId ? `reported_convo_${state.conversationId}` : null;
  const [alreadyReported, setAlreadyReported] = useState(false);

  useEffect(() => {
    if (missingState) {
      toast.error("We lost track of which conversation you were reporting.");
      navigate(-1);
      return;
    }
    try {
  ...
  const submit = async () => {
    ...
    setSubmitting(true);
    try {
      await api.post(`/api/reports/${state.conversationId}/conversation/${state.conversationId}`, {
        report_type: type,
      ...
      <div className="report-card">
        ...
        {alreadyReported ? (
          <div className="report-done">
            <p style={{color: 'var(--font-color)'}}>You've already reported this conversation. Our team will review it.</p>
